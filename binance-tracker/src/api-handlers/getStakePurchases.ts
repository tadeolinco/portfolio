...
export const getStakePurchases = async ({
  maxDate,
  csrftoken,
  pt20,
}: {
  ...
  console.log("Stake purchases: started");
  ...
  while (isAfter(currentDate, maxDate)) {
    const res = await axios.get(
      `https://www.binance.com/bapi/earn/v1/private/pos/union/getUserPurchaseRecord?`,
      {
        params: {
          pageIndex: page,
          pageSize: 20,
    ...
    for (const txn of res.data.data) {
      transactions.push({
        type: "STAKE_PURCHASE",
        orderNo: `${txn.asset}-${txn.createTimestamp}-${txn.amount}`,
        buyAsset: "",
        buyAmount: "0",
        buyFee: "0",
        buyFeeAsset: "",
  ...
  console.log("Stake purchases: done");
  return transactions;
};
