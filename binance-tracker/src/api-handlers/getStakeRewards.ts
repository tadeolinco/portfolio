...
export const getStakeRewards = async ({
  maxDate,
  csrftoken,
  pt20,
}: {
  ...
  console.log("Stake rewards: started");
  ...
  while (isAfter(currentDate, maxDate)) {
    const res = await axios.get(
      `https://www.binance.com/bapi/earn/v1/private/pos/union/getUserInterestRecord?`,
      {
        params: {
          pageIndex: page,
          pageSize: 20,
  ...
  console.log("Stake rewards: done");
  return transactions;
};
