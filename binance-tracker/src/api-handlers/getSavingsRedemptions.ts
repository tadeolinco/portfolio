...
export const getSavingsRedemptions = async ({
  maxDate,
  csrftoken,
  pt20,
}: {
  ...
  console.log("Savings redemptions: started");
  ...
  while (isAfter(currentDate, maxDate)) {
    ...
    for (const txn of res.data.data) {
      if (txn.status === "PAID") {
        transactions.push({
          type: "SAVINGS_REDEMPTION",
          orderNo: `${txn.projectName}-${txn.createTimestamp}-${txn.amount}`,
          buyAsset: txn.asset,
  ...
  console.log("Savings redemptions: done");
  return transactions;
};
